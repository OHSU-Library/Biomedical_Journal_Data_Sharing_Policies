...
knitr::opts_chunk$set(echo = params$showcode, warning = FALSE,message = FALSE,tidy=TRUE,fig.path = "figs/",fig.keep=TRUE)
...
```{r barplot_pct,fig.width=9,fig.height=7}
# 2013
tmp1 = jdata%>%group_by(dsm,dsm_fac)%>%
...
```{r barplot_pct_stacked,fig.width=9,fig.height=7}
...
for(ii in 2:ncol(tmp)) {
  tmp[,ii] = paste0(unlist(tmp[,ii])," (",round(tmppct[,ii],1),"%)")
}
