...
```{r data_process}
# read journal data
exceldat <- read_excel("../Data-Sharing-Policies_2017-01-18.xlsx", sheet = "Data",
                    na = "N/A")
...
ggplot(tmplong,aes(x=type1,y=Percent,fill=dsm_fac_reverse))+geom_bar(stat="identity") +
  ...
  scale_fill_manual(values = RColorBrewer::brewer.pal(n=16, name="Greys")[2:7],
...
```{r tile_percent_recDSM, fig.width=9,fig.height=7}
tmp = with(jdata,table(dsm_fac,rec_pref_sharing_fac))
tmp = cbind("dsm"=1:6,tmp)
tmp = tmp[-nrow(tmp),]
tmp = as_data_frame(tmp)
...
ggplot(tmplong,aes(dsm,type,fill=Percent))+geom_tile(show.legend=FALSE)+geom_text(aes(label=label),color="white")+
...
ggplot(tmplong,aes(dsm,type,fill=Percent))+
  geom_tile(show.legend=TRUE)+geom_text(aes(label=label),color="white")+
  theme_minimal()+ylab("")+xlab("Data Sharing Mark")+
  scale_fill_continuous(limits=c(0,100),low="lightgrey",high="black")
...
ggplot(tmplong,aes(x=dsm,y=impact_factor,alpha=year,fill=dsm_fac_reverse))+
  ...
  stat_summary(fun.y=mean,geom="point",position=position_dodge(.9),
               aes(x=dsm,y=impact_factor,shape="triangle"))+
  scale_shape_manual(values = 2)+
  scale_fill_manual(values = RColorBrewer::brewer.pal(n=16, name="Greys")[1+2:7],
                    breaks=levels(tmplong$dsm_fac),
                    labels=levels(tmplong$dsm_fac))+
  guides(alpha="none",shape="none",fill=guide_legend(title="Data Sharing Mark",ncol=1))+
  xlab("Data Sharing Mark (1-6), Year (2013 light, 2014 dark)")+
...
kw_results = data.frame(Y=c("if_2013_6catDSM","if_2014_6catDSM","if_2015_6catDSM",
                            "if_2013_2catDSM","if_2014_2catDSM","if_2015_2catDSM"),
                        bind_rows(
  broom::tidy(k_if13),broom::tidy(k_if14),broom::tidy(k_if15),
  broom::tidy(w_if13_2cat),broom::tidy(w_if14_2cat),broom::tidy(w_if15_2cat)
  )
  )
...
The pairwise Wilcoxon test p-values are below (by DSM group number) for 2013, 2014, and 2015.
...
```{r wilcoxon_res_2015,  results='asis'}
signif_15 = signif(w_if15$p.value,2)
signifind = which(signif_15<.05,arr.ind=T)
emphasize.strong.cells(signifind)
signif_15[which(signif_15<.001,arr.ind=T)] = "< 0.001"
...
## Kruskal-Wallis test for year effect
...
tmp2 = bind_rows(lapply(1:6, function(k){
  broom::tidy(with(jdata_long%>%filter(dsm==k),kruskal.test(impact_factor~as.factor(year))))
}))
tmp = bind_rows(tmp1,tmp2)
...
tmp2 = bind_rows(lapply(1:6, function(k){
  broom::tidy(with(jdata_long%>%filter(dsm==k),kruskal.test(citable_items~as.factor(year))))
}))
tmp = bind_rows(tmp1,tmp2)
tmp = cbind("DSM"=c("All",paste0("DSM = ",1:6)),tmp)
tmp%>%kable(digits=3)
...
In 2013, the total number of citable items in the set of studied journals was `r jdata_long%>%filter(year==2013)%>%select(citable_items)%>%sum%>%as.character`, in 2014 it was
`r jdata_long%>%filter(year==2014)%>%select(citable_items)%>%na.omit%>%sum%>%as.character` and in 2015 it was
`r jdata_long%>%filter(year==2015)%>%select(citable_items)%>%na.omit%>%sum%>%as.character`.
...
```{r}
tmpm = data.frame(
  matrix(tmp%>%filter(year==2015)%>%ungroup%>%select(`Total Citable`)%>%unlist,nrow=2))
tmpb = broom::tidy(chisq.test(tmpm))
if(tmpb$p.value < 2e-16) tmpb$p.value = "< 2e-16"
kable(tmpb)
```

## Trend test for proportion of citable items with required data sharing within open access categories across years

```{r}
tmp = jdata_long%>%group_by(year,oam_fac,dsm2)%>%
  summarize("Num Journals"=n(),"Total Citable"=sum(citable_items,na.rm=T))
...
```{r}
tmp = jdata_long%>%filter(!Journal=="PLoS One")%>%group_by(year,oam_fac,dsm2)%>%
  summarize("Num Journals"=n(),"Total Citable"=sum(citable_items,na.rm=T))
...
When looking at the open access journals including PLoS One, we can see that the proportion of data sharing required citable items is *decreasing* from 2013 to 2015 within open access journals (`r paste(round(tmpres_withPLOS[2,1:3],2),collapse=", ")`,   p=`r signif(tmpres_withPLOS$p.value[2],2)`). 

After removing PLos One, we can see that the proportion of data sharing required citable items is *increasing* from 2013 to 2015 within open access journals (`r paste(round(tmpres[2,1:3],2),collapse=", ")`,   p=`r signif(tmpres$p.value[2],2)`).

However, this proportion does not increase within subscription journals (`r paste(round(tmpres[1,1:3],2),collapse=", ")`, p=`r signif(tmpres$p.value[1],2)`).

The results are below:

```{r}
kable(cbind("PLOS"=c("withPLOS","withPLOS","withoutPLOS","withoutPLOS"),bind_rows(tmpres_withPLOS,tmpres)),digits=3)
```
...
In 2013, the total number of citable items in the set of studied journals (removing Plos One) was `r jdata_long%>%filter(year==2013)%>%select(citable_items)%>%sum%>%as.character`, in 2014 it was
`r jdata_long%>%filter(year==2014)%>%select(citable_items)%>%na.omit%>%sum%>%as.character`, and in 2015 it was
`r jdata_long%>%filter(year==2015)%>%select(citable_items)%>%na.omit%>%sum%>%as.character`
...
In order to understand the potential influence of the policies on the published literature, we also evaluated the distribution of policies by publication volume. In 2013, the total number of citable items (papers) in the studied journals was `r jdata_long%>%filter(year==2013)%>%select(citable_items)%>%sum%>%as.character` (after removing PlOS One), in 2014 the total number of citable items was `r jdata_long%>%filter(year==2014)%>%select(citable_items)%>%sum(.,na.rm=T)%>%as.character`
and in 2015 the total number of citable items was `r jdata_long%>%filter(year==2015)%>%select(citable_items)%>%sum(.,na.rm=T)%>%as.character`,

with median numbers of citable items per journal of 
`r jdata_long%>%filter(year==2013)%>%select(citable_items)%>%unlist%>%median(na.rm=T)`,
`r jdata_long%>%filter(year==2014)%>%select(citable_items)%>%unlist%>%median(na.rm=T)`, and 
`r jdata_long%>%filter(year==2015)%>%select(citable_items)%>%unlist%>%median(na.rm=T)`, respectively. 



# Sample Representation of All Journals

We can compare sampled journals to non-sampled journals ("out of sample"") or to the entire set of journals ("All Journals").

```{r}
jdata_all <- read_excel("../Data-Sharing-Policies_2017-01-18.xlsx", sheet = "2014 ESI Data",
                    na = "N/A")

jdata_all = jdata_all%>%rename(Journal=`Full Journal Title`,
                               if_2014=`Journal Impact Factor`,citable_2014=`Citable Items`,
                               total_cites_2014=`Total Cites`)

jdata_sample = jdata%>%select(Journal,ISSN,if_2014,citable_2014,total_cites_2014)%>%add_column("in_sample"="Sample")
```
...
```{r}
ggplot(jdata_all,aes(x=in_sample,y=if_2014,fill=in_sample))+geom_boxplot()+theme_minimal()+ggtitle("Distribution of Impact Factor")
ggplot(jdata_all_long,aes(x=in_sample,y=if_2014,fill=in_sample))+geom_boxplot()+theme_minimal()+ggtitle("Distribution of Impact Factor")

jdata_all_long%>%group_by(in_sample)%>%
  summarize_at(vars(if_2014),
               funs(Min="min",
                    Q25 = quantile(.,probs=.25),Mean="mean",Median="median",
                    Q75 = quantile(.,probs=.75),Max="max"),na.rm=T)%>%kable(digits=2)
```



## Distribution of Citable Items

```{r}

ggplot(jdata_all,aes(x=in_sample,y=citable_2014,fill=in_sample))+geom_boxplot()+theme_minimal()+ggtitle("Distribution of Citable Items")
ggplot(jdata_all%>%filter(citable_2014<20000),aes(x=in_sample,y=citable_2014,fill=in_sample))+geom_boxplot()+theme_minimal()+ggtitle("Distribution of Citable Items (remove Plos One)")

ggplot(jdata_all_long,aes(x=in_sample,y=citable_2014,fill=in_sample))+geom_boxplot()+theme_minimal()+ggtitle("Distribution of Citable Items")
ggplot(jdata_all_long%>%filter(citable_2014<20000),aes(x=in_sample,y=citable_2014,fill=in_sample))+geom_boxplot()+theme_minimal()+ggtitle("Distribution of Citable Items (remove Plos One)")


jdata_all_long%>%group_by(in_sample)%>%
  summarize_at(vars(citable_2014),
               funs(Total="sum",Min="min",
                    Q25 = quantile(.,probs=.25),Mean="mean",Median="median",
                    Q75 = quantile(.,probs=.75),Max="max"),na.rm=T)%>%kable(digits=2)
```
...
## Distribution ofTotal Citations

```{r}
ggplot(jdata_all,aes(x=in_sample,y=total_cites_2014,fill=in_sample))+geom_boxplot()+theme_minimal()+ggtitle("Distribution of Total Citations")
ggplot(jdata_all_long,aes(x=in_sample,y=total_cites_2014,fill=in_sample))+geom_boxplot()+theme_minimal()+ggtitle("Distribution of Total Citations")


jdata_all_long%>%group_by(in_sample)%>%
  summarize_at(vars(total_cites_2014),
               funs(Total="sum",Min="min",
                    Q25 = quantile(.,probs=.25),Mean="mean",Median="median",
                    Q75 = quantile(.,probs=.75),Max="max"),na.rm=T)%>%kable(digits=2)
```
...
```{r}
cdata1 <- read_excel("../DSP_Curator_Check.xlsx",sheet = "Curator Data")
cdata0 <- read_excel("../DSP_Curator_Check.xlsx",sheet = "Original Data")
cdata1 <- cdata1[-1,]
cdata0 <- cdata0[-1,]



cdata1_all = cdata1%>%clean_names()%>%remove_empty_rows()%>%
  select(journal,open_access_mark,data_sharing_mark:reproducibility_noted_mark)
cdata0_all = cdata0%>%clean_names()%>%remove_empty_rows()%>%
  select(journal,open_access_mark,data_sharing_mark:reproducibility_noted_mark)
...
for(tmpvar in allvars) {
  ...
  res = data_frame("Score"=tmpvar,
                   "# Journals"=nrow(newdat),
                   "% Agreement"=100*sum(diag(mytab))/nrow(newdat),
                   "Cohen's Kappa"=kappa2(newdat[,-1])$value)
